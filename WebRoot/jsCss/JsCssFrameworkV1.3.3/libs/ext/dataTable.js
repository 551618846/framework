// Copyright 2012~2013 Tencent Inc. All Rights Reserved.

/**
 * @表格控件(datatable, dt, grid)
...
(function (global, document, undefined) {
    ...
    DataTable.fn = DataTable.prototype = {
        ...
        createTable: function (options) {
            ...
            //存储展现的列到cookie (即allFields信息)
            if (options.cookie && options.cookie.saveFields) {
                var cookieName = options.tableId + "_visible_fields";

                if (options.cookie.cookieName) {
                    cookieName = options.cookie.cookieName;
                }

                if (this.isFirstLoad) {
                    //有cookie记忆则恢复最后一次状态
                    var fieldsCookie = this.getCookie(cookieName);
                    if (fieldsCookie.length > 0) {

                        if (options.cookie.handleFields) {
                            options.cookie.handleFields(options.allFields);
                        }
                    }
                }
                else {
                    ...
                    var fieldStr = this.objToJson(_fields);
                    this.setCookie(cookieName, fieldStr);
                }
            }
            ...
            if (ifDivNode) {
                ...
                if (document.all) {
                    ...
                    for (var key in allFields) {
                        ...
                        if (allFields[key] && allFields[key].width && parseInt(allFields[key].width) > 0) {
                        ...
                        try {
                            _th = document.createElement("<th name='" + key + "'></th>");
                        } catch (e) {
                        ...
                        if (options.thorder[key] != undefined) {
                            if (options.thorder[key] == 'desc') {
                    ...
                    for (var key in allFields) {
                        ...
                        if (key != 'gri_checkAll') {
                            ...
                            if (allFields[key].thText) {
                                ...
                                thStr += ' class="' + thClassName + '">' + allFields[key].thText;
                            } else {
                                thStr += ('<th name=' + key + ' ');
                                thStr += ' class="' + thClassName + '">' + key;
                            }
            ...
            if (!realTable.id) {
                realTable.id = this.options.tableId + "_table";
            }
            ...
            //单元格合并
            if (undefined != options.spanColIndex && options.data.length > 0) {
            ...
            if (options.autoTips) {
                ...
                _this.createAutoTips(options.tableId + "_table");
            }
        ...
        createPage: function () {
            ...
            if (this.page.index < this.page.count) {
                ...
                if (!!this.page.autoHide && parseInt(this.rowCount) <= parseInt(this.page.size)) {
        ...
        tableRowsSpan: function (TableName, ColList) {
            var ColArray = ColList.split(","), //指定要合并的列（例如："0,1"，0表示第一列）
                TableRowsCount = TableName.rows.length; //获取表格的总行数
            for (var j = ColArray.length - 1; j >= 0; j--) {//这里是倒着来的，先从后向前合并（针对列）
                ...
                for (var i = 0; i <= TableRowsCount; i++) {
                    if (i != TableRowsCount) {
                        var cell = TableName.rows[i].cells[ColArray[j]];
                        var CurId = cell ? cell.innerHTML : "";//获取表格i行j列的值
        ...
        setCookie: function (cookieName, value) {
            document.cookie = cookieName + "=" + encodeURIComponent(value) + ";expires=Sun, 18 Jan 2038 00:00:00 GMT;";
        },

        /**
         * 对象转化为Json字符串
         * @param {object} obj 需要转换的对象.
         * @return {string} 序列化之后的值.
         */
        ...
        createComplexHeader: function (complexHeader, table, _this) {
            ...
            for (var name in complexHeader) {
                ...
                for (var i = 0; i < _tr.length; i++) {
                    ...
                    if (document.all) {
                        try {
                            tH = document.createElement("<th name='" + field_name + "'></th>");
                        } catch (e) {
                            tH = document.createElement('TH');
                            tH.setAttribute('name', field_name);
                        }
                    }
                    else {
                        tH = document.createElement('TH');
                    ...
                    if (_tr[i].enable) {
                        ...
                        //表头初始化排序图标
                        var orderField = _this.options.noPage ? _this.options.noPage.orderField : _this.options.page.orderField;
                        var orderType = _this.options.noPage ? _this.options.noPage.orderType : _this.options.page.orderType;

                        var _iOrder = document.createElement("I");

                        if (_this.options.thorder[field_name] != undefined) {
                            if (_this.options.thorder[field_name] == 'desc') {
                                if (_this.options.page.ifRealPage) {
                                    _iOrder.className = 'icon-orderd';
                                }
                                else {
                            ...
                            if (field_name == orderField && _this.isFirstLoad) {
                                if (orderType == 'asc') {
                                    _iOrder.className = 'icon-orderu';
                                }
                                else {
                                ...
                                if (_this.options.allFields[field_name].needOrder != false && _this.options.enableThClick != false && field_name != 'gri_checkAll') {
                                    _iOrder.className = "icon-order-hover";
                                }
                            }
                        }

                        tH.appendChild(_iOrder);
                    }
                ...
                if (ths[i].innerHTML.indexOf("checkbox") < 0 && ths[i].className.indexOf('enable') > -1) {
                    (function (i) {
                        ...
                        if (_this.options.enableThClick != false) {
                            ...
                            var _thName = _th.getAttribute('name');

                            if (!_thName || _this.options.allFields[_thName].needOrder != false) {
                                _th.onclick = function () {
                                    ...
                                    //获取排序方式
                                    var order = _this.options.thorder[_thName];

                                    //如果没定义order，默认调整为正序(因为初始化的时候是倒序)
                                    if (!order) {
                                        order = 'asc';
                                        _this.options.thorder[_thName] = 'desc';
                                    }
                                        //如果是倒序，则变成正序
                                    else if (order == 'desc') {
                                        _this.options.thorder[_thName] = 'asc';
                                    }
                                        //如果是正序，则变成倒序
                                    else if (order == 'asc') {
                                        _this.options.thorder[_thName] = 'desc';
                                    ...
                                    for (var j = 0; j < ths.length; j++) {
                                        var _thOName = ths[j].getAttribute('name');
                                        if (_thOName && _thOName != _thName) {
                                            delete _this.options.thorder[_thOName];
                                        }
                                    }
                                    ...
                                    if (_field) {
                                        ...
                                        _this.page.orderType = _this.options.thorder[_thName];
                                    }
                                    ...
                                    if (_this.page.ifRealPage) {
                                        ...
                                        if (!_this.options.page && _this.options.data.length > 0 && _this.options.fixedRow) {
                                            var sortField = _field.sortField || _field.value;
                                            maxValue = _this.options.data[_this.options.data.length - 1][sortField];
                                            maxValue = _this.removeHTMLTag(maxValue);
                                        }
                                        ...
                                        _this.options.data.sort(function (row1, row2) {
                                            ...
                                            if (row1[sortField] == null) {
                                                col1 = '';
                                            }
                                            else {
                                                col1 = row1[sortField].toString();
                                            }

                                            if (row2[sortField] == null) {
                                                col2 = '';
                                            }
                                            else {
                                                col2 = row2[sortField].toString();
                                            }

                                            //去除可能存在的html标签
                                            col1 = _this.removeHTMLTag(col1);
                                            col2 = _this.removeHTMLTag(col2);
        ...
        /**
         * 截取元素的实际样式
         * @param {object} oElm 文档里的element.
         * @param {string} strCssRule CSS样式名.
         * @return {string}  结果值.
         */
         ...
         */
        createAutoTips: function (tableId) {
            var _this = this;
            var autoTips = document.getElementById('autotipsdiv');

            if (!autoTips) {
                autoTips = document.createElement('div');
                autoTips.id = 'autotipsdiv';
                autoTips.style.position = 'absolute';
                autoTips.style.display = 'none';
                autoTips.style.zIndex = '9999';
                document.body.appendChild(autoTips);

                /*
                 * 鼠标离开处理
                 */
                _this.addEvent(autoTips, 'mouseout', function (e) {
                    autoTips.style.display = 'none';
                });
            }
            ...
            var ths = _table.getElementsByTagName('th');

            _this.tipsMouseOver(tds);
            _this.tipsMouseOver(ths);
        },

        /**
         * 鼠标移入单元格内的处理
         * @param {object} elements 需要处理的元素
         */
        tipsMouseOver: function (elements) {
            var _this = this;
            var autoTips = document.getElementById('autotipsdiv');
            for (var i = 0; i < elements.length; i++) {
                (function (i) {
                    ...
                    _this.addEvent(_td, 'mouseover', function (e) {
                        autoTips.innerHTML = '';
                        autoTips.style.width = 'auto';

                        autoTips.style.paddingLeft = _this.getStyle(_td, 'padding-left');
                        autoTips.style.paddingLeft = _this.getStyle(_td, 'padding-top');
                        autoTips.style.fontSize = _this.getStyle(_td, 'font-size');
                        autoTips.style.lineHeight = _td.clientHeight + 'px';
                        autoTips.style.textAlign = _this.getStyle(_td, 'text-align');

                        var tdOffset = _this.getElementAbsPos(_td);
                        autoTips.style.left = tdOffset.left + "px";
                        autoTips.style.top = tdOffset.top + "px";

                        autoTips.innerHTML = _td.innerHTML;
                        autoTips.style.display = '';

                        if (autoTips.offsetWidth <= _td.offsetWidth) {
                            autoTips.style.display = 'none';
                        }
                        else {
